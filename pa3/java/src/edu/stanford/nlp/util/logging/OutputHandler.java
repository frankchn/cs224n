...
public abstract class OutputHandler extends LogRecordHandler{
  ...
  private void updateTracks(int untilDepth){
    while(!queuedTracks.isEmpty()){
      ...
      if(signal.depth >= untilDepth){ queuedTracks.push(signal); return; }
  ...
  public List<Record> handle(Record record) {
    ...
    String[] content;
    if (record.content instanceof Exception) {
      Exception exception = (Exception) record.content;
      List<StackTraceElement> stackTrace = Redwood.filterStackTrace(exception.getStackTrace());
      String exceptionDescription = record.content.toString();
      content = new String[stackTrace.size() + 1];
      content[0] = exceptionDescription;
      for (int i = 0; i < stackTrace.size(); i++) {
        content[i + 1] = tab + stackTrace.get(i).toString();
      }
  ...
  public List<Record> signalEndTrack(int newDepth) {
    ...
    if(this.queuedTracks.isEmpty()){
      ...
      if(time-childInfo.beginTime > 100){
        b.append("[");
