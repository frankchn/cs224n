...
# started:   1998-09-25
# version:   2004-01-26 [cdm mod for IO]
# author:    Erik Tjong Kim Sang <erikt@uia.ua.ac.be>

# cdm: This version adds an extra flag -i for scoring IO format
...
my $io = 0;               # raw IO input: add I to every token
...
while (@ARGV and $ARGV[0] =~ /^-/) {
   ...
   elsif ($ARGV[0] eq "-i") { $io = 1; shift(@ARGV); }
   elsif ($ARGV[0] eq "-d") { 
      shift(@ARGV); 
...
while (<STDIN>) {
   ...
   if ($raw) {
      ...
      if ($features[$#features-1] eq $oTag) { $features[$#features-1] = "O"; } 
      if ($features[$#features] ne "O") { 
         $features[$#features] = "I-$features[$#features]";
      }
      if ($features[$#features-1] ne "O") { 
         $features[$#features-1] = "I-$features[$#features-1]";
      }
   }
